...
function initBoard() {
    if ('!tests.boardValid');
    elseif ('!tests.cellsValid');
    displayMessage("These hints are designed to help you build your board object bit by bit. If you're seeing one, don't worry: you didn't do anything wrong, you're just not finished yet!</em>", 'notes')
    return null;
};
...
function cellsToNodes(boardNode, cell) {
    ...
    node.classList.add('row-3' + cell.row)
    node.classList.add('col-3' + cell.col)
    if (cell.isMine) {
